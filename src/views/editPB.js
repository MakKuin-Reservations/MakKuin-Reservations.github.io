import { getReservationPBById, updateReservationPB } from '../api/recipe.js';
...
export async function editPagePB(ctx) {
    const reservationId = ctx.params.id;
    const reservationPB = await getReservationPBById(reservationId);
    reservationPB.kidsCetaring = reservationPB.kidsCetaring.join('\n');
    reservationPB.parentMenu = reservationPB.parentMenu.join('\n');
    reservationPB.note2 = reservationPB.note2.join('\n');

    update();

    function update(errors = {}, data = reservationPB) {
    ...
    async function onSubmit(data, event) {
        try {
            ...
            const result = await updateReservationPB(reservationId, reservationPB);
            event.target.reset();
            ctx.notify('Резервацията е редактирана');
            ctx.page.redirect('/detailsPB/' + reservationId);
        } catch (err) {
            update(err, data);
        }
    }
