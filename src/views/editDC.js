import { getReservationDCById, updateReservationDC } from '../api/recipe.js';
...
export async function editPageDC(ctx) {
    const reservationId = ctx.params.id;
    const reservationDC = await getReservationDCById(reservationId);
    reservationDC.kidsCetaring = reservationDC.kidsCetaring.join('\n');
    reservationDC.parentMenu = reservationDC.parentMenu.join('\n');
    reservationDC.note2 = reservationDC.note2.join('\n');

    update();

    function update(errors = {}, data = reservationDC) {
    ...
    async function onSubmit(data, event) {
        try {
            ...
            const result = await updateReservationDC(reservationId, reservationDC);
            event.target.reset();
            ctx.notify('Резервацията е редактирана');
            ctx.page.redirect('/detailsDC/' + reservationId);
        } catch (err) {
            update(err, data);
        }
    }
