import { getReservationMovieById, updateReservationMovie } from '../api/recipe.js';
...
export async function editPageMovie(ctx) {
    const reservationId = ctx.params.id;
    const reservationMovie = await getReservationMovieById(reservationId);
    reservationMovie.kidsCetaring = reservationMovie.kidsCetaring.join('\n');
    reservationMovie.parentMenu = reservationMovie.parentMenu.join('\n');
    reservationMovie.note2 = reservationMovie.note2.join('\n');

    update();

    function update(errors = {}, data = reservationMovie) {
    ...
    async function onSubmit(data, event) {
        try {
            ...
            const result = await updateReservationMovie(reservationId, reservationMovie);
            event.target.reset();
            ctx.notify('Резервацията е редактирана');
            ctx.page.redirect('/detailsMovie/' + reservationId);
        } catch (err) {
            update(err, data);
        }
    }
