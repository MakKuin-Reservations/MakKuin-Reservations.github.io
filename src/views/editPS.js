import { getReservationPSById, updateReservationPS } from '../api/recipe.js';
...
export async function editPagePS(ctx) {
    const reservationId = ctx.params.id;
    const reservationPS = await getReservationPSById(reservationId);
    reservationPS.kidsCetaring = reservationPS.kidsCetaring.join('\n');
    reservationPS.parentMenu = reservationPS.parentMenu.join('\n');
    reservationPS.note2 = reservationPS.note2.join('\n');

    update();

    function update(errors = {}, data = reservationPS) {
    ...
    async function onSubmit(data, event) {
        try {
            ...
            const result = await updateReservationPS(reservationId, reservationPS);
            event.target.reset();
            ctx.notify('Резервацията е редактирана');
            ctx.page.redirect('/detailsPS/' + reservationId);
        } catch (err) {
            update(err, data);
        }
    }
