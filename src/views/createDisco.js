import { createReservationDisco } from '../api/recipe.js';
...
<section id="create">
    <article>
        ...
        <form @submit=${onSubmit} id="createForm">
            ...
            ${field({ label: 'Дата', name: 'date',type: 'datetime-local' ,value:"2022-01-01T00:00", error: errors.date })}
...
export function createDiscoPage(ctx) {
    update();
    ...
    async function onSubmit(data, event) {
        try {
            ...
            const result = await createReservationDisco(reservationDisco);
            event.target.reset();
            ctx.page.redirect('/detailsDisco/' + result.objectId);
        } catch (err) {
            update(err, data);
        }
    }
